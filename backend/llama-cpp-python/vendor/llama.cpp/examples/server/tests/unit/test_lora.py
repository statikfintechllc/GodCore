...
def test_lora(scale: float, re_content: str):
    global server
    server.start()
    res_lora_control = server.make_request(
        "POST", "/lora-adapters", data=[{"id": 0, "scale": scale}]
    )
...
def test_with_big_model():
    ...
    server.lora_files = [
        download_file(
            "https://huggingface.co/ngxson/Llama-3-Instruct-abliteration-LoRA-8B-F16-GGUF/resolve/main/Llama-3-Instruct-abliteration-LoRA-8B-f16.gguf"
        ),
    ...
    lora_config = [
        ...
        (
            [{"id": 0, "scale": 0.0}],
            "I can't provide you with a code for a computer virus",
        ),
        (
            [{"id": 0, "scale": 0.0}],
            "I can't provide you with a code for a computer virus",
        ...
        (
            [{"id": 0, "scale": 1.5}],
            "A task of some complexity! Here's a simple computer virus",
        ),
        (
            [{"id": 0, "scale": 1.5}],
            "A task of some complexity! Here's a simple computer virus",
