...
def test_chat_completion_stream(
    ...
    for data in res:
        ...
        assert (
            "gpt-3.5" in data["model"]
        )  # DEFAULT_OAICOMPAT_MODEL, maybe changed in the future
        if last_cmpl_id is None:
            last_cmpl_id = data["id"]
        assert (
            last_cmpl_id == data["id"]
        )  # make sure the completion id is the same for all events in the stream
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"type": "json_object", "schema": {"items": [{"type": "integer"}]}},
            10,
            "[ -3000 ]",
        ),
        (
            {"type": "json_schema", "json_schema": {"schema": {"const": "foooooo"}}},
            10,
            '"foooooo"',
        ),
...
def test_completion_with_grammar(
    jinja: bool, grammar: str, n_predicted: int, re_content: str
):
    global server
    server.jinja = jinja
    server.start()
    ...
    choice = res.body["choices"][0]
    assert match_regex(re_content, choice["message"]["content"]), choice["message"][
        "content"
    ]
