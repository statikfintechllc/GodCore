...
def do_test_completion_with_required_tool_tiny(
    server: ServerProcess, tool: dict, argument_key: str | None, n_predict, **kwargs
):
    ...
    ), f'Expected 1 tool call in {choice["message"]}'
    tool_call = tool_calls[0]
    assert choice["message"].get("content") in (
        None,
        "",
    ), f'Expected no content in {choice["message"]}'
    assert (
        len(tool_call.get("id", "")) > 0
    ), f"Expected non empty tool call id in {tool_call}"
    expected_function_name = (
        "python" if tool["type"] == "code_interpreter" else tool["function"]["name"]
    )
    ...
    if argument_key is not None:
        ...
        assert (
            argument_key in actual_arguments
        ), f"tool arguments: {json.dumps(actual_arguments)}, expected: {argument_key}"


@pytest.mark.parametrize(
    "template_name,tool,argument_key",
    [
...
def test_completion_with_required_tool_tiny_fast(
    ...
    do_test_completion_with_required_tool_tiny(
        server, tool, argument_key, n_predict, temperature=0.0, top_k=1, top_p=1.0
    )
...
def test_completion_with_required_tool_tiny_slow(
    template_name: str, tool: dict, argument_key: str | None
):
    global server
    n_predict = 512
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            TEST_TOOL,
            "success",
            "bartowski/Hermes-2-Pro-Llama-3-8B-GGUF:Q4_K_M",
        ...
        (
            ...
            ("meetkai/functionary-medium-v3.2", None),
        ),
        (
            PYTHON_TOOL,
            "code",
...
def test_completion_with_required_tool_real_model(
    ...
    hf_repo: str,
    template_override: str | Tuple[str, str | None] | None,
):
    global server
    n_predict = 512
    server.n_slots = 1
    ...
    if isinstance(template_override, tuple):
        ...
        ), f"Template file {server.chat_template_file} does not exist. Run `python scripts/get_chat_template.py {template_hf_repo} {template_variant} > {server.chat_template_file}` to download the template."
...
def do_test_completion_without_tool_call(
    server: ServerProcess,
    n_predict: int,
    tools: list[dict],
    ...
    **kwargs,
    ...
    choice = res.body["choices"][0]
    assert (
        choice["message"].get("tool_calls") is None
    ), f'Expected no tool call in {choice["message"]}'
...
def test_completion_without_tool_call_fast(
    template_name: str, n_predict: int, tools: list[dict], tool_choice: str | None
):
    global server
    server.jinja = True
    server.n_predict = n_predict
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "bartowski/functionary-small-v3.2-GGUF:Q8_0",
            ("meetkai/functionary-medium-v3.2", None),
        ),
...
def do_test_weather(server: ServerProcess, **kwargs):
    ...
    # assert choice["message"].get("content") in (None, ""), f'Expected no content in {choice["message"]}'
    assert (
        tool_call["function"]["name"] == WEATHER_TOOL["function"]["name"]
    ), f'Expected weather tool call, got {tool_call["function"]["name"]}'
    assert (
        len(tool_call.get("id", "")) > 0
    ), f"Expected non empty tool call id in {tool_call}"
    actual_arguments = json.loads(tool_call["function"]["arguments"])
    assert (
        "location" in actual_arguments
    ), f"location not found in {json.dumps(actual_arguments)}"
    location = actual_arguments["location"]
    assert isinstance(
        location, str
    ), f"Expected location to be a string, got {type(location)}: {json.dumps(location)}"
    assert re.match(
        "^Istanbul(( |, ?)(TR|Turkey|Türkiye))?$", location
    ), f"Expected Istanbul for location, got {location}"


@pytest.mark.slow
@pytest.mark.parametrize(
    "result_override,n_predict,hf_repo,template_override",
    [
...
def do_test_calc_result(
    server: ServerProcess, result_override: str | None, n_predict: int, **kwargs
):
    res = server.make_request(
        "POST",
        "/v1/chat/completions",
    ...
    if result_override is not None:
        assert re.match(
            result_override, content
        ), f"Expected {result_override}, got {content}"
    else:
        assert re.match(
            "^[\\s\\S]*?((That's|\\bis) (approximately )?)?\\b0\\.(5\\b|56\\b|556)",
            content,
...
@pytest.mark.parametrize(
    ...
    [
        (
            128,
            "deepseek",
            "^The sum of 102 and 7 is 109[\\s\\S]*",
            None,
        ...
        (
            1024,
            "deepseek",
            "To find the sum of[\\s\\S]*",
            "I need to calculate the sum of 102 and 7[\\s\\S]*",
            "bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF:Q4_K_M",
            None,
        ),
        (
            1024,
            "none",
            "^(<think>\\s*)?I need[\\s\\S]*?</think>\\s*To find[\\s\\S]*",
            None,
            "bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF:Q4_K_M",
            None,
        ),
...
def test_thoughts(
    n_predict: int,
    reasoning_format: Literal["deepseek", "none"] | None,
    expect_content: str | None,
    expect_reasoning_content: str | None,
    hf_repo: str,
    template_override: str | Tuple[str, str | None] | None,
):
    global server
    ...
    if expect_content is None:
        ...
        ), f"Expected {expect_content}, got {content}"

    reasoning_content = choice["message"].get("reasoning_content")
    if expect_reasoning_content is None:
        assert (
            reasoning_content is None
        ), f'Expected no reasoning content in {choice["message"]}'
    else:
        assert re.match(
            expect_reasoning_content, reasoning_content
        ), f"Expected {expect_reasoning_content}, got {reasoning_content}"
...
def test_hello_world(
    hf_repo: str, template_override: str | Tuple[str, str | None] | None
):
    global server
...
def do_test_hello_world(server: ServerProcess, **kwargs):
    ...
    actual_arguments = json.loads(tool_call["function"]["arguments"])
    assert (
        "code" in actual_arguments
    ), f"code not found in {json.dumps(actual_arguments)}"
    code = actual_arguments["code"]
    assert isinstance(
        code, str
    ), f"Expected code to be a string, got {type(code)}: {json.dumps(code)}"
    assert re.match(
        r"""print\(("[Hh]ello,? [Ww]orld!?"|'[Hh]ello,? [Ww]orld!?')\)""", code
    ), f"Expected hello world, got {code}"
